...
@section('content')
<div class="min-h-screen bg-neutral-bg flex flex-col pb-32">
    <!-- Header -->
    <header class="bg-neutral-card border-b border-neutral-border sticky top-0 z-40">
        <div class="max-w-6xl mx-auto px-16 py-12 flex justify-between items-center">
            <!-- Brand -->
            <div class="flex items-center gap-8">
                <div class="w-32 h-32 bg-brand-600 rounded-lg flex items-center justify-center text-white">
                    <svg class="w-16 h-16" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 .587l3.668 7.431 8.2 1.191-5.934 5.787 1.4 8.168L12 18.896l-7.334 3.857 1.4-8.168L.132 9.209l8.2-1.191L12 .587z"/>
                    </svg>
                </div>
                <span class="text-body-g font-bold text-neutral-primary">CP Review</span>
                <span class="text-neutral-secondary">|</span>
                <span class="text-body-m text-neutral-secondary">{{ $cliente->nome_empresa }}</span>
            </div>

            <!-- Logout -->
            <form action="{{ route('logout') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="border border-neutral-border hover:bg-neutral-bg px-16 py-8 rounded-lg text-body-m font-medium text-neutral-secondary transition">
                    Sair
                </button>
            </form>
        </div>
    ...
    <!-- Main Content -->
    <main class="flex-1 max-w-6xl w-full mx-auto px-16 py-24">
        @yield('cliente_content')
    </main>

    <!-- Bottom Navigation Bar (Sticky/Fixed bottom) -->
    <nav class="bg-neutral-card border-t border-neutral-border fixed bottom-0 left-0 right-0 z-50">
